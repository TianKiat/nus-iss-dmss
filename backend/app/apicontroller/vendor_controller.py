...
class VendorController:
    ...
    def get_menu_items(self, db, userId: UserID):
    ...
    def get_vendor_profile_id(self, db, userId: UserID):
        user_profile = VendorProfileService.get_vendor_profile(db, userId)
        if user_profile is None:
            return
    ...
    def get_orders(self, db, userId: UserID):
        user_profile = VendorProfileService.get_vendor_profile(db, userId)
        if user_profile is None:
            return []
    ...
    def update_opening_hours(self, db, openingHours : List[OpeningHoursModel]):
