...
function displayList(full_list, role_list, onChangeMethod, getNewControl){
    ...
    //     const full_list_id = Object.keys(full_list)

    //     const temp_list_ac = new Object();
    //     for (const temp in full_list_id){

    //         if (role_list.includes(parseInt(full_list_id[temp]))){
    //             temp_list_ac[full_list_id[temp]] = true;
    //         }else{
    //             temp_list_ac[full_list_id[temp]] = false;
    //         }
    //     }

    //     setCheckBoxState(temp_list_ac)
    // }
    
    // function customOnChange(key, isChecked){

    //     checkBoxState[key] = isChecked

    // }
    
    return(
        <>
            {/* <Checkbox isChecked={checkBoxState["1"]}></Checkbox> */}
            {/* {Object.keys(full_list).map(key=>{
                ...
                return(
                    ...
                    <Checkbox id={key} isChecked = {checkBoxState[key]} onChange={(e)=>customOnChange(key, e.target.checked)} > {full_list[key]}</Checkbox>
                )
            })} */}
            {/* <div>Hello</div>
            {
                ...
                <Stack>
                    {Object.keys(full_list).map(key=>{
                        ...
                        return(
                            ...
                            <Checkbox id={key} onChange={(e)=>onChangeMethod(key, e.target.checked)}> {full_list[key]}</Checkbox>
                        )
                    })}
            ...
            {Object.keys(full_list).map(key=>{
                ...
                console.log(role_list.includes(parseInt(key)))   ;  
                return(
                        role_list.includes(parseInt(key))?    
                        ...
                        <Checkbox id={key} onChange={(e)=>onChangeMethod(key, e.target.checked)} defaultChecked={true}> {full_list[key]}</Checkbox>                        
                        :
                        <Checkbox id={key} onChange={(e)=>onChangeMethod(key, e.target.checked)}> {full_list[key]}</Checkbox>
                    )
                })}
